...
function AuthScreen({ onSignedIn }: { onSignedIn: (user: SessionUser) => void }) {
  ...
  return <div className="marketing min-h-screen bg-[#f5f5f7] text-[#1d1d1f]">
    ...
    <main>
      ...
      <section className="mx-auto grid max-w-6xl grid-cols-2 gap-8 px-6 py-20 text-center md:grid-cols-4 md:py-28"><Metric value="10s" label="para registrar o ponto" /><Metric value="24/7" label="visão da jornada" /><Metric value="100%" label="em nuvem" /><Metric value="1 lugar" label="para toda a equipe" /></section>
      ...
      <section id="recursos" className="mx-auto max-w-7xl px-5 py-16 lg:px-8 lg:py-24"><p className="text-sm font-semibold text-[#0066cc]">FEITO PARA SER SIMPLES</p><h2 className="mt-4 max-w-4xl text-balance text-4xl font-semibold leading-tight tracking-[-0.045em] sm:text-6xl">Menos controle manual.<br /><span className="text-[#86868b]">Mais clareza para decidir.</span></h2><div className="mt-14 grid gap-5 lg:grid-cols-2">
        <FeatureCard className="bg-white" icon={<Smartphone />} eyebrow="Para o funcionário" title="Bater o ponto é só o começo." text="Uma tela direta, saldo atualizado e histórico claro. Sem treinamento, sem dúvida e sem planilha." visual={<div className="mx-auto mt-10 w-[260px] rounded-[42px] border-[8px] border-[#1d1d1f] bg-[#f5f5f7] p-5 shadow-2xl"><div className="mx-auto h-5 w-20 rounded-full bg-[#1d1d1f]" /><p className="mt-12 text-sm text-[#6e6e73]">Horas hoje</p><p className="mt-2 text-5xl font-semibold tracking-tight">06:42</p><div className="mt-8 rounded-2xl bg-[#0071e3] py-4 text-center text-sm font-semibold text-white">Registrar saída</div></div>} />
        <FeatureCard className="bg-[#1d1d1f] text-white" icon={<BarChart3 />} eyebrow="Para a gestão" title="A empresa inteira. Em um olhar." text="Acompanhe quem está trabalhando, horas do dia, saldos e exceções sem perseguir informação." dark visual={<div className="mt-10 grid grid-cols-3 gap-3"><MiniStat value="18" label="na equipe" /><MiniStat value="12" label="em turno" /><MiniStat value="142h" label="no mês" /></div>} />
...
function Metric({ value, label }: { value: string; label: string }) { return <div><p className="text-3xl font-semibold tracking-[-0.04em] sm:text-4xl">{value}</p><p className="mt-2 text-sm text-[#6e6e73]">{label}</p></div>; }
function MiniStat({ value, label }: { value: string; label: string }) { return <div className="rounded-2xl bg-white/10 p-4"><p className="text-2xl font-semibold">{value}</p><p className="mt-1 text-xs text-white/55">{label}</p></div>; }
...
function Brand() { return <div className="flex items-center gap-2.5"><svg className="size-8" viewBox="0 0 40 40" fill="none" aria-hidden="true"><circle cx="20" cy="20" r="18" fill="currentColor" /><path d="M13 21.5 18.1 26 28 14" stroke="var(--background, white)" strokeWidth="3.5" strokeLinecap="round" strokeLinejoin="round" /><circle cx="28" cy="14" r="2.2" fill="#2997ff" /></svg><span className="text-xl font-semibold tracking-[-0.04em]">simbi</span></div>; }
